...
<script>
  ...
  function check(){
    ...
    var flag=0;
    <% List<Student> stu = (List<Student>) session.getAttribute("SnoList");%>
    <%for (Student s : stu){%>
    var no = "<%=s.getUsername()%>";
    if (sno==no){
      flag=1;
    }
    <%}%>
...
<div class="container-fluid">
  ...
  <div class="container-fluid col row">
    <div class="col"style="padding: 30px 40px;border: #0b5ed7 solid ;border-radius: 10px">
      <form action="../ServletTeaIndex?action=stuinsert" method="post" class="was-validated">
        ...
        <div class="d-flex justify-content-between flex-row">
          <div class="col-5">
            ...
            <label class="form-label" for="build">宿舍楼：</label>
            <input class="form-control" for="build" name="build" id="build" value="<%=tea.getBuildid()%>|<%=tea.getBuildname()%>"></input>
          </div>
          <div class="col-5">
            <label class="form-label" for="room">安排宿舍：</label>
            <select class="form-control" name="room" id="room"  >
              <option>0|null</option>
              <%ArrayList<Room> Rooms= (ArrayList<Room>) session.getAttribute("TeaRoomList");
                for(Room bui:Rooms){%>
              <option><%=bui.getRoomid()%>|<%=bui.getRoomname()%></option>
              <%}
              %>
            </select>
          </div>
